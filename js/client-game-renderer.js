...
(function() {
  ...
  let gameStarted = false; // Track if game loop has started
  ...
  function startRenderer() {
    ...
    // Change to gameplay state but don't start the game loop yet
    ...
    // Don't start the game loop here - wait for countdown to finish
    // The game loop will be started by window.startGameLoop() after countdown
  }
  ...
  function startGameLoop() {
    if (gameStarted || !game) {
      console.log('[client-game-renderer] Game loop already started or game not ready', {gameStarted, game: !!game});
      return;
    }
    gameStarted = true;
  ...
  function renderServerState(ctx) {
    ...
    // Draw platforms
    if (serverGameState.platforms && serverGameState.platforms.length > 0) {
      ctx.fillStyle = '#8B4513'; // Brown color for platforms
      serverGameState.platforms.forEach(pf => {
        if (!pf) return;
        // Draw platform with a slight shadow effect
        ctx.fillRect(Math.floor(pf.x), Math.floor(pf.y), pf.w, pf.h);
        // Add a subtle highlight on top
        ctx.fillStyle = '#A0522D';
        ctx.fillRect(Math.floor(pf.x), Math.floor(pf.y), pf.w, 2);
        ctx.fillStyle = '#8B4513'; // Reset for next platform
      });
    }
